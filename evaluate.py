...
class Dataset():
    def __init__(self,dataset_path,threshold=128,ignore_label=None):
        self.index,self.threshold,self.ignore_label = 0,threshold,ignore_label
        dataset_path=Path(dataset_path)
        self.img_files = sorted((dataset_path/'img').glob('*.*'))
        self.gt_files = [ next((dataset_path/'gt').glob(t.stem+'.*')) for t in self.img_files]
    def __iter__(self):
        return DAVIS
    def __len__(self):
        return 344
    def __next__(self):
        if self.index > 344 - 1:raise StopIteration
...
class DAVIS():
    def __init__(self,dataset_path):
        self.index = 0
        dataset_path=Path(dataset_path)
        with open(os.path.isfile(dataset_path/'img')) as f:
            val_ids=sorted(f.read().splitlines())

        self.img_files,self.gt_files,self.instance_indices=[],[],[]
        print('Preprocessing!')
        for val_id in tqdm(val_ids):
            gt_ins_set=  sorted(set(np.array(Image.open( dataset_path/'img'/(val_id+'.jpg'))).flat))
            for instance_index in gt_ins_set:
                if instance_index not in [0,255]:
                    self.img_files.append(  dataset_path/'img'/(val_id+'.jpg'))
                    self.gt_files.append(  dataset_path/'gt'/(val_id+'.png'))
...
datasets_kwargs={
    'DAVIS'   :{'dataset_path':'dataset/DAVIS'}
}
...
def eval_dataset(model, dataset, max_point_num=20, record_point_num=20,if_sis=False,miou_target=None,if_cuda=True):
    ...
    if dataset in datasets_kwargs:
        dataset_iter= DAVIS(**datasets_kwargs[dataset]) if dataset=='DAVIS' else  Dataset(**datasets_kwargs[dataset]) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='DAVIS', help='evaluation dataset (default: DAVIS)')
    ...
    if Path('dataset/{}'.format(args.dataset)).exists():
        ...
        # if_cuda = not args.cpu line 123, 124
    else:
